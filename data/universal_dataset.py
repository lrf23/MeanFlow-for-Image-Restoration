...
class AlignedDataset_all(BaseDataset):
    ...
    def __init__(self, opt, image_size, augment_flip=True, equalizeHist=True, crop_patch=True, generation=False, task=None):
        ...
        if opt.phase == 'train':
            ...
            self.dir_Ablur = os.path.join(opt.dataroot, 'Deblur/' + opt.phase + '/input_augmented') # 可以尝试修改为（1:1）数据增强后的路径
            self.dir_Bblur = os.path.join(opt.dataroot, 'Deblur/' + opt.phase + '/target_augmented')
            ...
            self.dir_Arain_syn = os.path.join(opt.dataroot, 'syn_rain/' + opt.phase + '/Test1200/input') #Rain100H, Rain100L, Test100, Test1200,
            self.dir_Brain_syn = os.path.join(opt.dataroot, 'syn_rain/' + opt.phase + '/Test1200/target')   #Test2800
    ...
    def transform_img(self, img):
        ...
        Atransform = get_transform(self.opt, transform_params, grayscale=False)
        img = Atransform(img)
        return img
